...
class ProductController extends Controller
{
    ...
    public function almatyIn(Request $request)
    {
        ...
        }elseif (Auth::user()->type === 'kizilorgain'){
            $city_field = 'to_city';
            $city_value = 'Получено на складе в Кызылорде';
            $city = 'Кызылорда';
            $reg_field = 'reg_city';
    ...
    public function getInfoProduct(Request $request)
    {
        ...
        if ($track_code){
            $user_data = User::query()->select('name', 'surname', 'login', 'city', 'block', 'code')->where('id', $track_code->user_id)->first();
        }else{
            $user_data = [
                'name' => 'нет',
                'surname' => 'нет',
